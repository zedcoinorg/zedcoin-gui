# Anonymity Networks with Zedcoin

Currently only Tor and I2P have been integrated into Zedcoin. The usage of
...
is made or until `zedcoind` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `zedcoin-wallet-cli` and
`zedcoin-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `zedcoind` the ip/port of a socks proxy provided by a
...
which tells `zedcoind` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `zedcoind` the inbound address, network
...
which tells `zedcoind` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `zedcoind`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p" and
forwarded to `zedcoind` localhost port 30000.
...
`zedcoind` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/zedcoin
...
This will store key information in `/var/lib/tor/data/zedcoin` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/zedcoin/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Zedcoin network should have
similar timestamps.
...
### Intermittent Zedcoin Syncing

If a user only runs `zedcoind` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `zedcoind` as often as possible to conceal when transactions are being sent.
...
`zedcoind` currently selects two outgoing connections every 5 minutes for
...
`zedcoind` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `zedcoind` should likely disconnect
