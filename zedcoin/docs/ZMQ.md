# The Current/Future Status of ZMQ in Zedcoin

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Zedcoin
...
matching supported by both Zedcoin and ZMQ. The `format`, `context` and `event`
...
The Zedcoin daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Zedcoin daemon also ensures that
...
The Zedcoin daemon will send a `txpool_add` pub exactly once for each
