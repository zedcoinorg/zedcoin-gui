...
Core tests take longer than any other Zedcoin tests, due to the high amount of computational work involved in validating core components.
...
To run only Zedcoin's core tests (after building):
...
To run only Zedcoin's crypto tests (after building):
...
```bash
zedcoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `zedcoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Zedcoin's hash tests (after building):
...
To run only Zedcoin's performance tests (after building):
...
To run only Zedcoin's unit tests (after building):
